...
def display_convo():
    ...
    base64_image = "data:image/jpeg;base64,"+ get_base64_image('bot.png')

    with st.container():
        for i, message in enumerate(reversed(st.session_state.chat_history)):
            if i % 2 == 0:
                st.markdown(bot_template.replace("{{MSG}}", message).replace("{{BOT}}", base64_image), unsafe_allow_html=True)
            else:
...
@st.experimental_dialog("User Authentication")
def user_authentication_tab(arg):
...
def main():
    ...
    favicon_path = os.path.join(current_dir, 'favicon.jpeg')
    st.set_page_config(page_title="Stock Price AI Bot", page_icon=favicon_path)
    
    if "user_authentication_tab" not in st.session_state and not st.session_state.get('user_authenticated'):
        if st.button("Signin"):
            user_authentication_tab("A")
